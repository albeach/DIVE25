...
setup_production_certificates() {
    ...
    if ! command -v certbot &> /dev/null; then
        log "INFO" "Installing certbot and DNS plugin..."
        apt-get update
        apt-get install -y certbot python3-pip
        
        # Install the appropriate DNS plugin based on your provider
        # For example, for Route 53:
    ...
    certbot certonly \
        --dns-route53 \  # Replace with your DNS provider's plugin
        -d "${PROD_DOMAIN}" \
        -d "*.${PROD_DOMAIN}" \
        --agree-tos \
        --email "${CERT_EMAIL}" \
    ...
    if [ $? -eq 0 ]; then
        ...
        openssl pkcs12 -export \
            -in "/etc/letsencrypt/live/dive25/fullchain.pem" \
            -inkey "/etc/letsencrypt/live/dive25/privkey.pem" \
